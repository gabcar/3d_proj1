...
def generateNormalPdf(_shape=10, _range=[-2,2], _mu=[0,0], _sigma=[[0.5,0],[0,.5]], _res=[1024,1024]):
    """
    in:    ndarray (h,w)
...
def POCS(_im, _mask, _pdf, iter=16):
    ...
    for i in range(iter):
        ...
        im_cs = dwt2c(im_cs)
        im_cs = idwt2c(softThresh(im_cs, 0.025))
...
if __name__ == '__main__':
    ...
    #pdf = generateNormalPdf(_res=brain.shape)
    #mask = generateMask(pdf)
    #mask2 = generateMaskFromPng()
